...
describe('<Popover /> component', () => {
  it('should renders with necessary components and children', () => {
    ...
    expect(statelessPopover.find('.DraftTextEditPopover-InnerWrapper')).toHaveLength(1);
    expect(statelessPopover.find('.DraftTextEditPopover-Body')).toHaveLength(1);
  });
  ...
  it('should adds correct classes when `hasArrow`, `placementX` or `placementY` are given', () => {
    ...
    expect(statelessPopover.hasClass('DraftTextEditPopover_hasArrow')).toBeTruthy();
    expect(statelessPopover.hasClass('DraftTextEditPopover_placementX_left')).toBeTruthy();
    expect(statelessPopover.hasClass('DraftTextEditPopover_placementY_top')).toBeTruthy();
  });
